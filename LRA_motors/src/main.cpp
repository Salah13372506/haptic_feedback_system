...
// Configuration WiFi
const char* ap_ssid = "HapticArduino";     // Nom du réseau que va créer l'Arduino
...
// Tableau des moteurs
Haptic_DRV2605 haptic[6];

// Buffer pour les données UDP
WiFiUDP udp;
...
void handleHapticCommand(int motor_id, int waveform, bool activate, float intensity) {
  if (motor_id < 0 || motor_id > 5) return;
